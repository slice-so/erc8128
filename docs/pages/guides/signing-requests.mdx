...
// For signing only specific components
// (e.g., authorization that works across multiple endpoints)
const signed = await signRequest(
  ...
  signer,
  {
    ...
    replay: 'replayable',        // Often combined
  }
)
```
...
:::warning
Class-bound signatures are less secure. Only use when you understand the trade-offs.
:::
