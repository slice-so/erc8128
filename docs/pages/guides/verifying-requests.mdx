...
When a request contains multiple signatures (multiple labels in the headers), the verifier evaluates them in priority order:

1. **Request-bound signatures first** — Verified before class-bound signatures
2. **Less restrictive class-bound policies** — When multiple class-bound policies match, the one with fewer required components is tried first
3. **Header order** — Within the same priority tier, signatures are tried in the order they appear in the headers

The first signature that passes all checks (cryptographic verification, time bounds, nonce, component policy) is accepted. This prioritization ensures the most specific authorization is used when available, while still accepting broader authorizations when configured.
