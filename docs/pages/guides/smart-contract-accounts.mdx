...
Use the package helper to keep ERC-6492 first, prevent ECDSA fallback for
code-bearing accounts, and enforce canonical EOA signatures:
...
const verifyMessage = createUniversalAccountVerifier({
  ...
  verifySmartAccount: async ({
    ...
    return data?.slice(0, 10) === '0x1626ba7e'
  },
})
...
The smart-account callback handles ERC-6492 and ERC-1271. Code-free accounts
use the package's strict local EOA recovery instead.
