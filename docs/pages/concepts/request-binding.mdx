...
When `requiredComponents` is set, it allows class-bound requests with the specified components, in addition to the default request-bound behaviour. The verifier checks that the specified components appear as an ordered subsequence of the signed components.
