...
```
Signature-Input: request=("@scheme" "@authority" "@method" "@path" "@query" "content-digest");...
```
...
The Fetch `Headers` API joins repeated field lines before the signer sees them.
For a covered component using RFC 9421's `;bs` parameter, this implementation
splits that joined value at top-level commas and encodes each part separately.
It is therefore interoperable only when each original field line is itself
comma-free. A comma within one field line is indistinguishable from the join
between repeated lines and is encoded as multiple byte sequences.
...
// Signature covers: @scheme + @authority + @method + @path + @query + content-digest + x-idempotency-key + x-request-id
```
