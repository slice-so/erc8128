...
When `requiredComponents` is set, it **replaces** the default request-bound check entirely. The verifier only checks that the specified components appear as an ordered subsequence of the signed components.

This means setting `requiredComponents` is the mechanism for **accepting class-bound signatures**. By specifying a subset of the default components, you tell the verifier exactly which components you require and accept signatures that cover at least those.
...
- **Set to `['@authority', '@method', '@path', '@query', 'content-digest']`**: The verifier checks for this exact ordered subsequence regardless of the request shape. A `GET` request without a body would still require `content-digest` to be signed, which is unlikely to be present.
...
- You have application-specific headers (like a scope or authorization context) that should be cryptographically protected
...
The required components must appear in the same relative order within the signed components, but don't need to be contiguous. Extra components in the signature are allowed.
