...
The signer chooses to produce a class-bound signature, but **the verifier decides whether to accept it**. By default, verifiers reject class-bound signatures — they must explicitly opt in via `requiredComponents`. See [Request Binding](/concepts/request-binding#class-bound-signatures) for details.

:::warning
Class-bound weakens request integrity. Only accept when the unsigned components don't affect authorization.
:::
...
Non-replayable is the baseline — all compliant verifiers must accept it. Replayable acceptance is optional and up to the verifier. See [Replay Protection](/concepts/replay-protection) for the full tradeoff analysis.
