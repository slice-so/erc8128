...
```
Signature-Input: request=(...);created=1618884473;expires=1618884533;...
```
...
- `request_not_yet_valid` — `now < created` (clock skew issue)
- `request_expired` — `now > expires`
- `request_validity_too_long` — `expires - created > maxValiditySec`
- `invalid_time` — Invalid time values (e.g., `expires <= created`)
...
```
Signature-Input: request=(...);nonce="a1b2c3d4-e5f6-7890";...
```
...
Custom replay keys must derive only from covered signature material such as the provided `keyid` and `nonce`. Do not close over unsigned request state, and ensure every verifier backend participates in a single logical nonce namespace; otherwise switching backends can make an already-consumed nonce appear unused.
...
2. **Keep validity windows short** — Choose the shortest window the delivery path and clock uncertainty allow. The library defaults to 60 seconds (`ttlSeconds`) for interoperability, but the standard does not prescribe that signer default
