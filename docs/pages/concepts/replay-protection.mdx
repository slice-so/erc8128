...
- **No nonce state** — Verifiers do not need a nonce store (e.g., Redis), simplifying infrastructure. This is significant for stateless architectures or edge deployments where shared state is expensive.
...
const result = await verifyRequest(request, {
  nonce: 'optional',  // Accept with or without nonce
  verifyMessage,
})
```
...
2. **Keep validity windows short** — For non-replayable signatures, 5–30 seconds is common (just enough for clock skew and network latency). For replayable signatures, the window depends on how long the signature should remain valid — minutes for session-like access, but keep it as short as practical. The library defaults to 60 seconds (`ttlSeconds`), but adjust based on your use case
