...
Verification policy with rules for validation. Signatures are verified in the order they appear in `Signature-Input` after filtering to EIP-8128 keyIds and allowed policies. Use `maxSignatureVerifications` to cap how many candidates are tried (default: 3).
