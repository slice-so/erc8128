...
type VerifyPolicy = {
   ...
   * The signer decides whether to produce a request-bound or class-bound signature.
   * When not set, the verifier enforces the default request-bound check 
   * (derived from request shape), which rejects class-bound signatures.
   * When set, only the listed components must appear as an ordered 
   * subsequence — this allows the verifier to accept class-bound signatures 
   * while still requiring specific components. Request-bound signatures 
   * also pass this check since they include more components.
   * Can include custom headers (e.g., 'x-idempotency-key').
   */
