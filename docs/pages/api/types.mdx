...
type VerifyPolicy = {
   ...
   * When not set, enforces full request-bound check (rejects class-bound signatures).
   * When set, replaces the request-bound check: only the listed components 
   * must appear as an ordered subsequence. This is how you accept class-bound signatures.
