...
type VerifyPolicy = {
  ...
  /** Direct-account verification policy (default: "universal"). */
...
type DelegationPolicy = {
  ...
  revocation: {
    ...
    verify: (context: DelegationRevocationContext) =>
      | "valid"
      | "revoked"
      | "epoch-mismatch"
      | "unavailable"
...
The revocation authority is trusted verifier configuration, not signed input.
Unknown delegation Dictionary members are rejected before this callback runs.
...
| `principal` | `AccountIdentity` | Authenticated root or direct account |
...
| `delegationId` | `Uint8Array` | The 32-byte authorization identifier on delegated results |
| `delegationEpoch` | `number` | The root issuance epoch signed into the authorization |
| `scopes` | `string[]` | Standard case-sensitive authorization scopes |
...
```typescript
type SignerClientOptions = Omit<SignOptions, "replay"> & {
  fetch?: typeof fetch
