...
export const client = createClient(signer)
```
...
On the server, call `verifyRequest` with a nonce store and a message verification function. The result contains the authenticated address and chain ID.
