...
describe("formatKeyId", () => {
  ...
  test("lowercases the address", () => {
    expect(formatKeyId(1, "0xAbCdEf0000000000000000000000000000000001")).toBe(
      "eip8128:1:0xabcdef0000000000000000000000000000000001"
    )
  })

  test("supports large chain IDs", () => {
    expect(formatKeyId(137, "0x0000000000000000000000000000000000000001")).toBe(
      "eip8128:137:0x0000000000000000000000000000000000000001"
    )
  })
...
describe("parseKeyId", () => {
  ...
  test("parses with large chain ID", () => {
    const result = parseKeyId(
      "eip8128:42161:0xabcdef0000000000000000000000000000000001"
    )
    expect(result).toEqual({
      chainId: 42161,
  ...
  test("returns null for non-eip8128 prefix", () => {
    expect(
      parseKeyId("not-eip8128:1:0x0000000000000000000000000000000000000001")
    ).toBeNull()
  })

  test("returns null for missing address", () => {
    expect(parseKeyId("eip8128:1:")).toBeNull()
  })

  test("returns null for short address", () => {
    expect(parseKeyId("eip8128:1:0x0001")).toBeNull()
  })

  test("returns null for address without 0x prefix", () => {
    expect(
      parseKeyId("eip8128:1:0000000000000000000000000000000000000001")
    ).toBeNull()
  })
  ...
  test("returns null for missing chain ID", () => {
    expect(
      parseKeyId("eip8128::0x0000000000000000000000000000000000000001")
    ).toBeNull()
  })
