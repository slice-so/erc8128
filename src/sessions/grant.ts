...
const canonicalAddress = (value: string) => {
  if (!addressPattern.test(value)) throw new Error("Session grant address is invalid.")
...
const canonicalScopes = (values: readonly string[]) => {
  ...
  const sorted = [...values].sort((left, right) =>
    utf8Encode(left).toString().localeCompare(utf8Encode(right).toString())
  )
...
export const parseSessionGrantMessage = (message: string): SessionGrant | null => {
  if (utf8Encode(message).length > maximumMessageBytes || message.includes("\r")) return null
  ...
  const field = (index: number, prefix: string) =>
    lines[index]?.startsWith(prefix) ? lines[index]?.slice(prefix.length) : undefined
  ...
  if (
    appOrigin === undefined || audience === undefined || account === undefined ||
    sessionSigner === undefined || scopesValue === undefined || nonce === undefined
  ) return null
  try {
    ...
    return {
      account: canonicalAddress(account), appOrigin, audience, chainId,
      expiresAt, issuedAt, nonce,
      scopes: scopesValue === "" ? [] : scopesValue.split(" "),
      sessionSigner: canonicalAddress(sessionSigner), signerScheme: "eoa", version: 1
    }
  } catch { return null }
}
...
export const validateSessionGrant = (
  ...
  return (
    ...
    (expected.sessionSigner === undefined || grant.sessionSigner === canonicalAddress(expected.sessionSigner)) &&
    grant.scopes.length === expectedScopes.length &&
    grant.scopes.every((scope, index) => scope === expectedScopes[index]) &&
    grant.issuedAt <= now + skew && grant.expiresAt >= now - skew &&
